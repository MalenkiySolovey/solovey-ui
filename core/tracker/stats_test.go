...
func TestStatsTrackerRejectsConnectionThroughIPObserver(t *testing.T) {
	observer := &testIPObserver{allow: false}
	tracker := NewStatsTracker(observer)
	raw := newBlockingTestConn()
	wrapped := tracker.RoutedConnection(context.Background(), raw, adapter.InboundContext{Inbound: "in", User: "alice"}, nil, fakeStatsOutbound{tag: "out"})
	if wrapped != raw {
