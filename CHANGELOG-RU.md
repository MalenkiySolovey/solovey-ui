...
- Регистронезависимый поиск клиентов; дефолты TLS-опций больше не «протекают»
  между формами; «Сохранить» во входящих недоступна, пока конфиг невалиден;
...
- **Заголовок страницы в верхней панели.** Каждая страница показывает свой
  заголовок, счётчик-сводку (например, «Входящие: 8 • Онлайн: 3») и поле поиска
...
## Что нового в v1.5.7 (бета) — обзор для пользователей

Понятная сводка всего, что добавлено со времён последнего стабильного релиза
**v1.5.6**. Полные примечания по релизам:
...
- **Встроенная оплата через 6 провайдеров** — Telegram Stars, YooKassa, Stripe,
  CryptoBot, PayMaster и внешние ссылки — с безопасным продлением без двойного списания.
...
- **Гибкая маршрутизация Telegram** — прокси (HTTP/HTTPS/SOCKS5) или sing-box-аутбаунд,
...
- **Корректные ссылки при bulk-правке.** Правка группы клиентов с *разными*
  наборами inbound’ов теперь пересобирает ссылки каждого клиента из его
  собственных inbound’ов, а не копирует набор первого.
