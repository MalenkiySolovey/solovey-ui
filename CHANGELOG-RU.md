...
## [1.5.5-beta4] — 2026-05-26 — стабильность, безопасность и импорт

### 1. Стабильность системы и защита от гонок данных

- Синхронизация потоков и устранение data races (П. 20, 21, 22, 28, 47, 48):
  операции обновления Telegram-клиента, retry-таймеры notifier, cooldown
  перезапуска ядра и фоновый сброс token-use теперь синхронизированы и
  корректно переживают переинициализацию БД и lifecycle API-тестов.
  Влияние: панель не падает с паникой и не повреждает состояние при
  параллельных запросах, интенсивной нагрузке или перезапуске ядра.
- Идемпотентный старт и более управляемая SQLite-нагрузка (П. 14, 15, 19):
  дефолтные настройки вставляются безопасно на уровне БД, ошибки
  post-migration adapt останавливают запуск, а лимиты SQLite pool можно
  настроить через переменные окружения.
  Влияние: исключены дубликаты настроек при конкурентном старте, а проблемы
  миграции больше не маскируются предупреждением в логах.

### 2. Безопасность и защита от утечек

- Защита токенов и авторизации (П. 33, 34): WS-токены проверяются без
  измеримой утечки по времени, а устаревший HTTP-заголовок `Token` получил
  жёсткий Sunset с отказом после даты отключения.
  Влияние: снижена поверхность тайминг-атак и закреплён переход на
  `Authorization: Bearer`.
- Изоляция секретов и сетевых данных (П. 23, 24, 25, 30, 31): системная
  информация больше не раскрывает приватные и link-local IP, Telegram backup
  зануляет секреты в памяти, optional URL settings отбрасывают unsafe/control
  input, а WARP-запросы используют единый безопасный набор headers.
  Влияние: меньше шансов раскрыть внутреннюю топологию, парольные данные или
  принять вредный URL в настройках.
- Защита import-xui от DoS (П. 36): rate-limit cache получил ограничение
  размера и очистку устаревших buckets.
  Влияние: поток запросов с уникальных IP больше не может бесконечно раздувать
  память процесса.

### 3. Целостность данных и отказоустойчивость

- Блокировка тихих ошибок (П. 1, 16, 17, 18, 26): ошибки удаления TLS-записей,
  чтения `client_ips` и commit статистики теперь обрабатываются явно; audit
  writer сохраняет приоритет warning/security событий, а лишний шум при
  legacy secretbox decrypt убран.
  Влияние: transient DB-сбой приводит к безопасному отказу или видимой ошибке,
  а не к молчаливой потере конфигурации, статистики или security-событий.
- Надёжность резервного копирования (П. 10, 11, 12): SIGHUP timeout стал
  настраиваемым, WAL checkpoint получил fallback `TRUNCATE -> FULL`, а restore
  versioned backup с отсутствующим `settings.config` продолжает работу с
  предупреждением.
  Влияние: backup/restore устойчивее на медленных дисках и нестандартных
  старых БД.

### 4. Импорт 3x-ui, cron sync и API-контракты

- Корректная семантика `reset_required` (П. 2, 8, 13, 46): импорт админов
  теперь сохраняет `force_password_reset` без генерации и показа нового
  пароля, а UI использует тот же контракт, что backend.
  Влияние: админов можно импортировать с обязательной сменой пароля без утечки
  временных credential в отчёте.
- Политики синхронизации теперь сохраняются и исполняются (П. 3, 7):
  `OnlyNew`, include settings/history/routing и `adminMode` проходят из
  профиля cron sync в планирование и apply.
  Влияние: запланированный sync больше не игнорирует настройки, выбранные
  администратором.
- Import-xui стал устойчивее на больших и долгоживущих операциях (П. 6, 35,
  37, 38, 39): v1/v2 routes регистрируются из одного источника, большие
  apply-планы читаются потоково, старые temp-директории очищаются, rollback
  публикует realtime invalidation, а WireGuard endpoint skip считается в
  правильном счётчике.
  Влияние: меньше расхождений между API-версиями, меньше ручной чистки temp и
  корректнее UI-состояние после rollback.
- Cron sync стал наблюдаемее и спокойнее к временным сбоям (П. 4, 5, 40, 41):
  ошибки пишутся с sanitized detail и error class, retry использует
  экспоненциальный backoff, а сбой записи success-summary не превращает уже
  выполненный импорт в hard-failure.
  Влияние: оператор видит причину сбоя и не получает ложную ошибку после
  успешного применения данных.

### 5. Frontend UX и realtime-поведение

- Realtime fallback сам пытается восстановить WebSocket-соединение (П. 32,
  42). Влияние: после краткого сетевого сбоя UI возвращается из polling-mode
  без ручной перезагрузки страницы.
- MigrateXui показывает ошибку apply inline, ждёт health после rollback перед
  reload и требует явного reveal для сгенерированных admin-паролей (П. 43,
  44, 45). Влияние: меньше риск потерять важное сообщение или случайно
  показать credential на экране.
- Сохранение Endpoint защищено от double-submit и корректно сбрасывает loading
  state при ошибке. Влияние: повторный клик по Save больше не запускает две
  конкурирующие операции.

### 6. Релизная упаковка

- Версия и workflow defaults обновлены до `v1.5.5-beta4` для Release, Windows
  и Docker сборок.
- Публичные release notes и changelog приведены к формату пользовательских
  изменений, без внутренних служебных отчётов.
