...
- **Исправление:** все действия записи `/api/paidsub/*` (привязки, тарифы,
  рассылка) слались как form-urlencoded, а бэкенд ждёт JSON — в beta2 каждая
  запись молча падала; теперь шлётся JSON и всё работает.
...
- **Исправление:** утечка соединений в цикле опроса бота (idle-соединения
  выброшенных proxy/outbound-транспортов теперь закрываются).
...
- **Выбор транспорта Telegram для каждого модуля.** Бот «Платных подписок» и
  ...
  **sing-box-аутбаунд** (нужно запущенное ядро); настраиваются независимо.
...
- **Исправления (UI beta1):** выпадающий список инбаундов в *Auto-registration*
...
- **Саморегистрация с пробным периодом:** неизвестный пользователь, открывший
  бота, может быть автоматически зарегистрирован с выбранными админом inbound'ами
...
- Конфиг-источник только с DNS (без правил маршрутизации, без proxy-outbound'ов и
...
## [1.5.6-beta8] — 2026-06-03 — миграция 3x-ui: аутбаунды, матчеры маршрутизации, TLS-сертификаты и DNS

- Прокси-аутбаунды теперь мигрируют как аутбаунды s-ui: раньше обрабатывались
  ...
  отбрасывалась, и цепочечный/прокси-аутбаунд пропадал из перенесённой панели.
  Теперь каждый превращается в полноценный аутбаунд sing-box (сервер/порт,
  ...
  аутбаунд вместо «требует ручной проверки».
- Системные аутбаунды сопоставляются со своим аналогом в sing-box:
  `freedom`→`direct`, `blackhole`→`block`, а аутбаунд `dns` превращается в
  действие маршрута `hijack-dns` (в sing-box нет аутбаунда `dns`). `loopback` и
...
- Никакой тихой потери при выключенном импорте роутинга: прокси/WARP аутбаунды
  лежат в исходном `xrayConfig`, который читается только при импорте роутинга.
  Если импорт роутинга выключен, но в источнике есть аутбаунды, план теперь
...
- Аутбаунды создаются только если они новые (повторный импорт или
  ...
  аутбаунд с тем же тегом), а в отчёте об импорте появляется счётчик `outbounds`
...
- Не-reality TLS-сертификаты теперь мигрируют: инбаунд, у которого в
...
- Доп. для аутбаундов: `packet_encoding` (`xudp`) выставляется, если источник
  его использовал; аутбаунд с несколькими серверами превращается в членов
...
- Trojan-инбаунд больше не роняет ядро: редактор инбаунда писал верхнеуровневый
  `password`, который sing-box у Trojan-инбаунда отвергает (`unknown field
  "password"` — аутентификация идёт по `users`, по клиентам). Поле пароля теперь
  только для аутбаундов, а оставшийся верхнеуровневый `password` срезается при
  сборке конфига (так что уже созданные инбаунды чинятся без правки).
...
- У перенесённых клиентов появляются ссылки подписки: импортёр оставлял поле
  `Links` пустым, а подписка читает только сохранённые ссылки, поэтому инбаунды
  ...
  `Links` теперь допустим — редактирование инбаунда перегенерирует ссылки, а не
  пропускает клиента.
...
- Исправляет полностью ломающий баг во встроенном импорте 3x-ui (`migrate-xui`):
  ...
  инбаунды, клиенты, WireGuard-эндпоинты, reality TLS (с дедупликацией),
  маршруты и история.
...
- Делает безопасной миграцию между разными хостами/доменами: серверо- и
  ...
  может включить любой пункт на шаге проверки. Инбаунд, привязанный к
