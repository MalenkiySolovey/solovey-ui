...
REM Solovey UI Windows Control Script
REM This script provides a menu-driven interface for managing Solovey UI on Windows

cd /d "%~dp0"
set "SERVICE_NAME=solovey-ui"
set "INSTALL_DIR=%SUI_HOME%"
if "%INSTALL_DIR%"=="" set "INSTALL_DIR=C:\Program Files\Solovey UI"

:menu
cls
echo ========================================
echo Solovey UI Windows Control Panel
echo ========================================
echo.
echo Current directory: %INSTALL_DIR%
echo.
echo 1. Start Solovey UI Service
echo 2. Stop Solovey UI Service
echo 3. Restart Solovey UI Service
echo 4. Check Service Status
echo 5. View Service Logs
echo 6. Open Panel in Browser
echo 7. Run Solovey UI Manually
...
:start_service
echo Starting Solovey UI service...
...
:stop_service
echo Stopping Solovey UI service...
...
:restart_service
echo Restarting Solovey UI service...
...
:check_status
echo Checking Solovey UI service status...
...
:open_panel
echo Opening Solovey UI panel in browser...
...
:run_manual
echo Running Solovey UI manually...
if exist "%INSTALL_DIR%\sui.exe" (
    cd /d "%INSTALL_DIR%"
    echo Starting Solovey UI in current window...
    echo Press Ctrl+C to stop
    echo.
    sui.exe
) else (
    echo Solovey UI executable not found: %INSTALL_DIR%\sui.exe
...
if exist "%INSTALL_DIR%\solovey-ui-service.exe" (
    cd /d "%INSTALL_DIR%"
...
if exist "%INSTALL_DIR%\sui.exe" (
    ...
    echo Solovey UI executable not found. Please run the installer first.
)
pause
goto menu
...
:exit
echo Thank you for using Solovey UI Windows Control Panel!
exit /b 0
