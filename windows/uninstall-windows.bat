...
echo ========================================
echo Solovey UI Windows Uninstaller
echo ========================================
...
echo Uninstalling Solovey UI from: %INSTALL_DIR%
...
if exist "%INSTALL_DIR%\solovey-ui-service.exe" (
    ...
    cd /d "%INSTALL_DIR%"
    solovey-ui-service.exe uninstall >nul 2>&1
    if %errorLevel% equ 0 (
...
if exist "%DESKTOP%\Solovey UI.lnk" (
    del "%DESKTOP%\Solovey UI.lnk" >nul 2>&1
    echo Desktop shortcut removed
)

REM Remove Start Menu shortcut
echo Removing Start Menu shortcut...
set "START_MENU=%APPDATA%\Microsoft\Windows\Start Menu\Programs\Solovey UI"
if exist "%START_MENU%" (
...
if /i "%keep_data%"=="y" (
    ...
    if exist "%INSTALL_DIR%\solovey-ui-service.exe" del "%INSTALL_DIR%\solovey-ui-service.exe" >nul 2>&1
    if exist "%INSTALL_DIR%\solovey-ui-service.xml" del "%INSTALL_DIR%\solovey-ui-service.xml" >nul 2>&1
...
netsh advfirewall firewall delete rule name="Solovey UI Panel" >nul 2>&1
netsh advfirewall firewall delete rule name="Solovey UI Subscription" >nul 2>&1
...
echo ========================================
echo.
echo Solovey UI has been uninstalled from your system.
echo.
if /i "%keep_data%"=="y" (
...
echo.
echo Thank you for using Solovey UI!
echo.
pause
