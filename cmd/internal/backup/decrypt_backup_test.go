...
func TestDecryptBackupCommandRoundTripWithEnvPassphrase(t *testing.T) {
	...
	envelope, err := integrationtelegram.BuildTelegramBackupEnvelope(payload, []byte(passphrase))
	if err != nil {
		t.Fatal(err)
	}
	...
	code := RunDecrypt([]string{"--in", inPath, "--out", outPath, "--passphrase-env", "SUI_TEST_PASSPHRASE"}, strings.NewReader(""), &stdout, &stderr, func(name string) string {
...
func TestDecryptBackupCommandRoundTripWithStdinPassphrase(t *testing.T) {
	...
	var stdout, stderr bytes.Buffer
	code := RunDecrypt([]string{"--in", inPath, "--out", outPath, "--passphrase-stdin"}, strings.NewReader(passphrase+"\n"), &stdout, &stderr, os.Getenv)
	if code != 0 {
		t.Fatalf("unexpected exit code %d stderr=%s", code, stderr.String())
	}
	got, err := os.ReadFile(outPath)
...
func TestDecryptBackupCommandWrongPassphraseRemovesPartialOutput(t *testing.T) {
	...
	inPath := filepath.Join(dir, "backup.db.aes")
	outPath := filepath.Join(dir, "backup.db")
	envelope, err := integrationtelegram.BuildTelegramBackupEnvelope([]byte("payload"), []byte(passphrase))
	if err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(inPath, envelope, 0o600); err != nil {
...
func TestDecryptBackupCommandRejectsPassphraseArgument(t *testing.T) {
	...
	passphrase := "correct horse battery staple"
	var stdout, stderr bytes.Buffer
	code := RunDecrypt([]string{"--in", inPath, "--out", outPath, "--passphrase", passphrase}, strings.NewReader(""), &stdout, &stderr, os.Getenv)
	if code != 2 {
		t.Fatalf("unexpected exit code %d stderr=%s", code, stderr.String())
	}
	if strings.Contains(stdout.String(), passphrase) || strings.Contains(stderr.String(), passphrase) {
