<template>
  <v-card :subtitle="$t('objects.listen')">
    ...
    <v-row>
      ...
      <v-col cols="12" sm="6" md="4">
        <v-switch v-model="proxyProtocol" color="primary" :label="$t('listen.proxyProtocol')" hide-details></v-switch>
      </v-col>
    </v-row>
...
export default {
  ...
  computed: {
    ...
    proxyProtocol: {
      get(): boolean { return this.$props.data.proxy_protocol === true },
      set(v:boolean) { v ? this.$props.data.proxy_protocol = true : delete this.$props.data.proxy_protocol }
    },
    optionTcpKeepAlive: {
      get(): boolean {
