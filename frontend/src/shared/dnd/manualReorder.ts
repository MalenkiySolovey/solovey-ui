...
export type ManualOrderPersistence = (object: string, order: Array<number | string>) => Promise<boolean>

let persistManualOrder: ManualOrderPersistence | undefined

export const configureManualOrderPersistence = (persist: ManualOrderPersistence) => {
  persistManualOrder = persist
}

export const resetManualOrderPersistence = () => {
  persistManualOrder = undefined
}

const saveManualOrder = async (object: string, order: Array<number | string>): Promise<boolean> => {
  if (!persistManualOrder) {
    throw new Error('manual order persistence is not configured')
  }
  return persistManualOrder(object, order)
}
...
export const moveManualOrder = async <T extends Record<string, any>>(
  ...
  return saveManualOrder(object, rows.map(row => Number(row[key])))
}
...
export const sortManualOrderByText = async <T extends Record<string, any>>(
  ...
  return saveManualOrder(object, rows.map(row => Number(row[idKey])))
}
