...
import {
  configureManualOrderPersistence,
  dragManualOrder,
...
describe('manualReorder', () => {
  ...
  it('persists manual order through an injected persistence function', async () => {
    const persist = vi.fn().mockResolvedValue(true)
    configureManualOrderPersistence(persist)

    await expect(dragManualOrder('outbounds', [{ id: 1 }, { id: 2 }], 1, 2)).resolves.toBe(true)

    expect(persist).toHaveBeenCalledWith('outbounds', [2, 1])
  })
})
