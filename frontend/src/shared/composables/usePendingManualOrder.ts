...
export const usePendingManualOrder = <T extends Record<string, any>>(
  ...
  const moveTo = (draggedId: RowId, targetId: RowId, position: ManualDropPosition | null = null): boolean => {
    const rows = moveRowsToPosition(displayItems.value, [draggedId], targetId, position, idKey)
    if (!rows) return false
    setOrder(rows)
    return true
  }

  const moveManyTo = (draggedIds: RowId[], targetId: RowId, position: ManualDropPosition | null = null): boolean => {
    const rows = moveRowsToPosition(displayItems.value, draggedIds, targetId, position, idKey)
    if (!rows) return false
    setOrder(rows)
    return true
  }
