...
// BatchSize returns the largest batch size that keeps a multi-row
...
func BatchSize(db *gorm.DB, modelValue any) int {
...
// CreateInBatches inserts slice into the given table while keeping each
...
func CreateInBatches(tx *gorm.DB, slice any) error {
	if tx == nil {
		return nil
	}
	...
	batch := BatchSize(tx, elem)
	return tx.CreateInBatches(slice, batch).Error
}

// SaveInBatches runs gorm.Save in chunks small enough for SQLite. Save
...
func SaveInBatches(tx *gorm.DB, slice any) error {
	if tx == nil {
		return nil
	}
	v := reflect.ValueOf(slice)
