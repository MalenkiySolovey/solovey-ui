...
func TestExportIncludesServicesAndTokens(t *testing.T) {
...
func TestExportPreservesNoTLSForeignKeySentinel(t *testing.T) {
	dbDir := t.TempDir()
	...
	if err := dbsqlite.DB().Create(&model.Inbound{
		Type:    "http",
		Tag:     "backup-no-tls",
		Addrs:   []byte("[]"),
		OutJson: []byte("{}"),
...
func TestExportCopiesTLSRowsWhenNoTLSSentinelExists(t *testing.T) {
	dbDir := t.TempDir()
	dbPath := filepath.Join(dbDir, "s-ui.db")
	t.Setenv("SUI_DB_FOLDER", dbDir)
	if err := dbsqlite.Init(dbPath); err != nil {
	...
	backup, err := Export("")
	if err != nil {
		t.Fatalf("Export failed with tls.id=0 sentinel and real TLS row: %v", err)
	}
	backupPath := filepath.Join(t.TempDir(), "backup.db")
	if err := os.WriteFile(backupPath, backup, 0o600); err != nil {
		t.Fatal(err)
...
func TestExportUsesRandomTempPathAndRemovesIt(t *testing.T) {
	t.Setenv("SUI_DB_FOLDER", t.TempDir())
	if err := dbsqlite.Init(filepath.Join(t.TempDir(), "s-ui.db")); err != nil {
		if strings.Contains(err.Error(), "go-sqlite3 requires cgo") {
			t.Skip(err)
	...
	if _, err := os.Stat(tempPath); !os.IsNotExist(err) {
		t.Fatalf("backup temp file was not removed after Export returned: %v", err)
	}
}

func TestExportExcludeSkipsSelectedTables(t *testing.T) {
	dbDir := t.TempDir()
	dbPath := filepath.Join(dbDir, "s-ui.db")
	t.Setenv("SUI_DB_FOLDER", dbDir)
	if err := dbsqlite.Init(dbPath); err != nil {
	...
	backup, err := Export("audit,client_ips,stats,changes")
	if err != nil {
		t.Fatal(err)
	}
	backupPath := filepath.Join(t.TempDir(), "backup.db")
...
func TestExportHandlesHashedClientIPsWithEmptyLegacyIP(t *testing.T) {
	if runtime.GOOS == "windows" {
		// Same Windows-specific TempDir cleanup race documented in
		// backup_import_test.go: SQLite/WAL leftovers occasionally hold the
		// temp directory open past closeMainDB. The behaviour exercised
...
`).Error; err != nil {
	...
	backup, err := Export("")
	if err != nil {
		t.Fatalf("Export failed on hashed client_ips with empty legacy ip: %v", err)
	}
	backupPath := filepath.Join(t.TempDir(), "backup.db")
	if err := os.WriteFile(backupPath, backup, 0600); err != nil {
		t.Fatal(err)
...
func TestExportHandlesLargeTablesWithoutVariableLimit(t *testing.T) {
	dbDir := t.TempDir()
	dbPath := filepath.Join(dbDir, "s-ui.db")
	t.Setenv("SUI_DB_FOLDER", dbDir)
	if err := dbsqlite.Init(dbPath); err != nil {
	...
	backup, err := Export("")
	if err != nil {
		t.Fatalf("Export failed on large dataset: %v", err)
	}
	backupPath := filepath.Join(t.TempDir(), "backup.db")
	if err := os.WriteFile(backupPath, backup, 0600); err != nil {
		t.Fatal(err)
