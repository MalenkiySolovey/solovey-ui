...
func BenchmarkBackupExport(b *testing.B) {
	runBackupExportBench(b, 100_000)
}

func BenchmarkBackupLargeExport(b *testing.B) {
	runBackupExportBench(b, 1_000_000)
}

func runBackupExportBench(b *testing.B, rowsPerTable int) {
...
func seedBackupBenchSmallTables(db *gorm.DB) error {
	...
	return db.CreateInBatches(&clients, dbsqlite.BatchSize(db, &model.Client{})).Error
}
...
func initBackupBenchLiveDB(tb testing.TB, livePath string) {
	...
	tb.Cleanup(closeBackupBenchDB)
}

func closeBackupBenchDB() {
	_ = dbsqlite.Close()
}
