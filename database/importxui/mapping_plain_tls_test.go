...
func tlsTestInbound(stream string) source.InboundRow {
	return source.InboundRow{
		ID:             1,
		Tag:            "tls-in",
		Protocol:       "vless",
		Port:           443,
...
func TestExtractPlainTLS_InlineArrayAndSingle(t *testing.T) {
	...
	spec, warnings, err := mapping.ExtractPlainTLS(rowArr)
	if err != nil {
		t.Fatal(err)
	}
	if spec == nil {
	...
	rec, err := mapping.BuildPlainTLSRecord(*spec)
	if err != nil {
		t.Fatal(err)
	}
	...
	specStr, _, err := mapping.ExtractPlainTLS(rowStr)
...
func TestExtractPlainTLS_NonTLSAndReality(t *testing.T) {
	if spec, _, err := mapping.ExtractPlainTLS(tlsTestInbound(`{"security":"reality","realitySettings":{}}`)); err != nil || spec != nil {
		t.Fatalf("reality must yield no plain-tls spec: spec=%v err=%v", spec, err)
	}
	if spec, _, err := mapping.ExtractPlainTLS(tlsTestInbound(`{"security":"none"}`)); err != nil || spec != nil {
