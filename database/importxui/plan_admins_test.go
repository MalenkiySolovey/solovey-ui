...
func TestIssue2ResetRequiredMergeAndReplaceKeepUserRowsStable(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if err := dbsqlite.DB().Model(&model.User{}).Where("id = ?", before.Id).Updates(map[string]any{
				"password":             localHash,
...
func TestIssue2NewPasswordClearsForceResetAndReturnsGeneratedAdmin(t *testing.T) {
	...
	before := clusterEUserByName(t, "admin")
	if err := dbsqlite.DB().Model(&model.User{}).Where("id = ?", before.Id).Update("force_password_reset", true).Error; err != nil {
		t.Fatal(err)
	}
...
func clusterEUserByName(t *testing.T, username string) model.User {
	t.Helper()
	var user model.User
	if err := dbsqlite.DB().Where("username = ?", username).First(&user).Error; err != nil {
		t.Fatal(err)
	}
	return user
}
