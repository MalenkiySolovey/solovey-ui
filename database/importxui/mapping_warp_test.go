...
func TestMapXrayOutbounds_WARP(t *testing.T) {
	endpoints, _, targets, _ := mapping.MapXrayOutbounds(warpXrayConfig)

	if targets["blocked"] != mapping.RejectTarget || targets["direct"] != mapping.DirectOutboundTag || targets["warp"] != "warp" {
...
func TestMapXrayRouting_WARPRuleTargetsEndpoint(t *testing.T) {
	_, _, targets, _ := mapping.MapXrayOutbounds(warpXrayConfig)
	mapped, warnings, mappedCount, manualCount := mapping.MapXrayRouting(warpXrayConfig, targets)
...
func TestCreateNewEndpoints_IdempotentNoClobber(t *testing.T) {
	...
	endpoints, _, _, _ := mapping.MapXrayOutbounds(warpXrayConfig)
	report := &Report{}
	...
	endpoints2, _, _, _ := mapping.MapXrayOutbounds(warpXrayConfig)
	report2 := &Report{}
...
func TestWarpEndpoint_DropsMalformedReserved(t *testing.T) {
	...
	endpoints, _, _, warnings := mapping.MapXrayOutbounds(cfg)
	if len(endpoints) != 1 {
