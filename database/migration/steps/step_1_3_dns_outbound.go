...
func removeOutboundStrategy(db *gorm.DB) error {
...
func backfillAnyTLSUserConfig(db *gorm.DB) error {
...
func normalizeDNSAndOutboundOptions(db *gorm.DB) error {
	err := backfillAnyTLSUserConfig(db)
	if err != nil {
		return err
	}
	err = migrateDNS(db)
