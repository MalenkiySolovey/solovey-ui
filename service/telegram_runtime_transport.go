...
func newCoreOutboundHTTPClient(tag string, timeout time.Duration) (*http.Client, error) {
	return newCoreOutboundHTTPClientForRuntime(DefaultRuntime(), tag, timeout)
}

func newCoreOutboundHTTPClientForRuntime(runtime *Runtime, tag string, timeout time.Duration) (*http.Client, error) {
	if tag == "" {
