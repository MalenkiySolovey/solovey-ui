...
func TestGetLogsFilteredValidatesAndFilters(t *testing.T) {
	...
	query, err := opsdiagnostics.ParseLogQuery("1000", "INFO", "panel", "")
	if err != nil {
		t.Fatal(err)
	}
	if query.Count != opsdiagnostics.MaxLogCount || query.Level != "info" {
...
func TestGetLogsFilteredRejectsInvalidInputs(t *testing.T) {
	tests := []struct {
		...
		{name: "filter length", filter: strings.Repeat("a", opsdiagnostics.MaxLogFilter+1)},
...
func TestParseLogQueryRejectsInvalidCategory(t *testing.T) {
	if _, err := opsdiagnostics.ParseLogQueryWithCategory("10", "debug", "", "", "kernel"); err == nil {
