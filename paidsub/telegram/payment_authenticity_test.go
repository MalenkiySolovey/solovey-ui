...
func TestHandlePreCheckoutRejectsInvalid(t *testing.T) {
	...
	db.Create(&paid.PaymentOrder{ClientId: 1, TariffId: 1, Provider: string(provider.ProviderStars), Amount: 100, Currency: "XTR", Status: paid.StatusPending, TelegramUserId: 7, IdempotencyKey: "pc"})
	db.Create(&paid.PaymentOrder{ClientId: 1, TariffId: 1, Provider: string(provider.ProviderStars), Amount: 100, Currency: "XTR", Status: paid.StatusPaid, TelegramUserId: 7, IdempotencyKey: "pc-paid"})
...
func TestHandleSuccessfulPaymentRefusesMismatch(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			db.Create(&client)
			order := paid.PaymentOrder{ClientId: client.Id, TariffId: 1, Provider: string(provider.ProviderStars), Amount: 100, Currency: "XTR", Status: paid.StatusPending, TelegramUserId: 7, IdempotencyKey: "sp"}
			db.Create(&order)
...
func TestHandleSuccessfulPaymentRefusesWrongTelegramUser(t *testing.T) {
	...
	db.Create(&client)
	order := paid.PaymentOrder{ClientId: client.Id, TariffId: 1, Provider: string(provider.ProviderStars), Amount: 100, Currency: "XTR", Status: paid.StatusPending, TelegramUserId: 7, IdempotencyKey: "spw"}
	db.Create(&order)
...
func TestHandleSuccessfulPaymentUnknownPayloadIsNoop(t *testing.T) {
	...
	order := paid.PaymentOrder{ClientId: 1, TariffId: 1, Provider: string(provider.ProviderStars), Amount: 100, Currency: "XTR", Status: paid.StatusPending, TelegramUserId: 7, IdempotencyKey: "real"}
	db.Create(&order)
