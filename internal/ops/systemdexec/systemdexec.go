// Package systemdexec owns the systemd control executable authority. It exposes
// only a verified executable object; argv and environment remain with the
// semantic systemd owner.
package systemdexec
...
var ErrUnavailable = errors.New("systemd control executable is unavailable")

type Object struct{ object *executableobject.Object }

func Open() (*Object, error) {
	return openCandidates([]string{"/usr/bin/systemctl", "/bin/systemctl"})
}
...
func openCandidates(candidates []string) (*Object, error) {
	...
	for _, candidate := range candidates {
		...
		if left.Device != right.Device || left.Inode != right.Inode || left.Digest != right.Digest {
			_ = selected.Close()
			_ = object.Close()
			return nil, errors.New("systemd control executable authority is ambiguous")
		}
		_ = object.Close()
	}
	if selected != nil {
