...
type Host struct {
	...
	systemdAnalyze      *executableobject.Object
	checkpointAuthority broker.CompletedMutationAuthority
	now                 func() time.Time
}
...
var errFixedRootOwnedExecutableUnavailable = errors.New("fixed root-owned executable is unavailable")

func registerSystemdHandlers(registry *broker.Registry, checkpointAuthority broker.CompletedMutationAuthority) error {
	if checkpointAuthority == nil {
		return broker.StartupFailure("deployment", "deployment.checkpoint", "systemd", "authority_unavailable", errors.New("deployment checkpoint recovery authority is required"))
	}
	systemctl, err := systemdexec.Open()
	if err != nil {
		return broker.StartupFailure("deployment", "service.supervision", "systemd", "required_executable_unavailable", err)
	}
	systemdAnalyze, _ := fixedBinary("/usr/bin/systemd-analyze", "/bin/systemd-analyze")
	host := &Host{systemctl: systemctl, systemdAnalyze: systemdAnalyze, checkpointAuthority: checkpointAuthority, now: time.Now}
	definitions := []struct {
		verb     broker.Verb
		mutation bool
		...
		{broker.VerbDeploymentRelease, true, host.releaseCheckpointHandler},
	...
	for _, value := range definitions {
		lifecycle := completedMutationLifecycle(value.verb)
		if err := registry.Register(value.verb, broker.Definition{Role: broker.RolePanel, Mutation: value.mutation,
			...
			return broker.StartupFailure("deployment", "deployment.lifecycle", "systemd", "handler_registration_failed", err)
		}
	}
	return nil
}
...
func (h *Host) prepareHandler(ctx context.Context, envelope broker.Request, _ broker.PeerIdentity) (any, error) {
	...
	if posture.Profile == request.TargetProfile || posture.Runtime != domain.RuntimeSystemdNative {
...
func (h *Host) releaseCheckpointHandler(_ context.Context, envelope broker.Request, _ broker.PeerIdentity) (any, error) {
	var request ReleaseCheckpointRequestV1
	if broker.DecodePayload(envelope.Payload, &request) != nil || !digest(request.CheckpointRef) {
		return nil, broker.Failure(broker.CodeInvalidRequest, "deployment checkpoint release identity is invalid")
	}
	if err := authoritativePreparedCheckpoint(h.checkpointAuthority, envelope.OperationID, request.CheckpointRef); err != nil {
		return nil, broker.Failure(broker.CodeFence, "deployment checkpoint release authority differs")
	}
	checkpoint, err := readCheckpoint(request.CheckpointRef)
	if err == nil && (checkpoint.OperationID != envelope.OperationID || checkpoint.Revision != request.CheckpointRef) {
		return nil, broker.Failure(broker.CodeFence, "deployment checkpoint release authority differs")
	}
	if err != nil {
		path := filepath.Join(checkpointRoot, request.CheckpointRef+".json")
		if _, statErr := os.Lstat(path); !errors.Is(statErr, os.ErrNotExist) {
			return nil, broker.Failure(broker.CodeFence, "deployment checkpoint release authority differs")
		}
		if syncErr := syncAbsentCheckpoint(checkpointRoot); syncErr != nil {
			return nil, broker.Failure(broker.CodeInternal, "deployment checkpoint absence is not durable")
		}
	} else if err := removeCheckpoint(request.CheckpointRef); err != nil {
		return nil, broker.Failure(broker.CodeInternal, "deployment checkpoint could not be released")
	}
	return ReleaseCheckpointResultV1{CheckpointRef: request.CheckpointRef, ProviderRevision: ProviderRevision}, nil
}

func authoritativePreparedCheckpoint(authority broker.CompletedMutationAuthority, operationID, checkpointRef string) error {
	if authority == nil || !digest(checkpointRef) {
		return errors.New("deployment completed prepare authority is unavailable")
	}
	response, err := authority.CompletedMutation(operationID, broker.VerbDeploymentPrepare, "deployment-profile")
	if err != nil || !response.OK || response.PayloadDigest != broker.Digest(response.Payload) {
		return errors.New("deployment completed prepare authority is unavailable")
	}
	var prepared PrepareResultV1
	if broker.DecodePayload(response.Payload, &prepared) != nil || prepared.ProviderRevision != ProviderRevision || prepared.CheckpointRef != checkpointRef {
		return errors.New("deployment completed prepare authority differs")
	}
	return nil
}

func syncAbsentCheckpoint(root string) error {
	if info, err := os.Lstat(root); err == nil && info.IsDir() && info.Mode()&os.ModeSymlink == 0 {
		return syncDir(root)
	} else if !errors.Is(err, os.ErrNotExist) {
		return errors.New("deployment checkpoint root is unsafe")
	}
...
func (h *Host) observe(ctx context.Context) (domain.Posture, []byte, error) {
	...
	if !ok || definition.Runtime != domain.RuntimeSystemdNative {
	...
	posture := domain.Posture{Schema: domain.SchemaV1, Profile: profile, InstalledProfile: profile, ActiveProfile: activeProfile, Runtime: domain.RuntimeSystemdNative,
...
func installedMarkerProfile() (domain.ProfileID, bool) {
	...
	definition, ok := domain.Lookup(profile)
	return profile, ok && definition.Runtime == domain.RuntimeSystemdNative
}
...
func (h *Host) run(ctx context.Context, args ...string) ([]byte, error) {
	if h == nil || h.systemctl == nil || h.systemctl.File() == nil || h.systemctl.Revalidate() != nil {
		return nil, errors.New("systemd control executable object is unavailable")
	}
	bounded, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	command := exec.CommandContext(bounded, h.systemctl.ExecPath(0), args...)
	command.Args[0] = h.systemctl.Label()
	command.ExtraFiles = []*os.File{h.systemctl.File()}
...
func (h *Host) directiveCapability(ctx context.Context, unitPath, versionOutput string) (domain.Availability, string) {
	if h == nil || h.systemdAnalyze == nil || unitPath != activeUnit && !knownProfilePath(unitPath) {
		return domain.Unknown, domain.Revision("systemd-analyze-unavailable")
	}
	unit, _, unitErr := secureRootFile(unitPath, 128<<10)
	analyzerRevision, analyzerErr := h.systemdAnalyze.Identity().Digest, h.systemdAnalyze.Revalidate()
	...
	if versionOK && version >= 249 {
		command := exec.CommandContext(bounded, h.systemdAnalyze.ExecPath(0), "verify", "--man=no", "--recursive-errors=yes", unitPath)
		command.Args[0] = h.systemdAnalyze.Label()
		command.ExtraFiles = []*os.File{h.systemdAnalyze.File()}
		command.Env = []string{"PATH=/usr/sbin:/usr/bin:/sbin:/bin", "LANG=C", "LC_ALL=C"}
		var buffer limitBuffer
		command.Stdout, command.Stderr = &buffer, &buffer
		err := command.Run()
...
func detectProfile(properties map[string]string) domain.ProfileID {
	if data, _, err := secureRootFile(profileMarker, 128); err == nil {
		...
		if profile, ok := domain.Lookup(value); ok && profile.Runtime == domain.RuntimeSystemdNative {
			return value
		}
	}
...
func writeCheckpoint(checkpoint checkpointV1) error {
	return writeCheckpointAt(checkpointRoot, checkpoint)
}

func writeCheckpointAt(root string, checkpoint checkpointV1) error {
	...
	if existing, readErr := readCheckpointAt(root, checkpoint.Revision); readErr == nil {
		if existing.Revision == checkpoint.Revision && existing.OperationID == checkpoint.OperationID {
			return nil
		}
		return errors.New("deployment checkpoint identity conflicts")
	} else if _, statErr := os.Lstat(filepath.Join(root, checkpoint.Revision+".json")); !errors.Is(statErr, os.ErrNotExist) {
		return errors.New("deployment checkpoint path is unsafe")
	}
	if err := cleanupCheckpointTemporaries(root); err != nil {
		return err
	}
	count, bytes, err := checkpointStoreUsage(root)
	if err != nil || count >= maxCheckpoints || bytes+int64(len(data)) > maxCheckpointStoreBytes {
		return errors.New("deployment checkpoint retention bound requires operator recovery")
	}
	return atomicWrite(filepath.Join(root, checkpoint.Revision+".json"), data, 0o600, 0, 0)
}

func removeCheckpoint(revision string) error {
	return removeCheckpointAt(checkpointRoot, revision)
}

func removeCheckpointAt(root, revision string) error {
	if !digest(revision) {
		return errors.New("deployment checkpoint reference is invalid")
	}
	path := filepath.Join(root, revision+".json")
...
func cleanupCheckpointTemporaries(root string) error {
	...
	removed := false
	for _, entry := range entries {
		if !strings.HasPrefix(entry.Name(), ".solovey-deployment-") {
			continue
		}
		info, err := entry.Info()
		stat, ok := itemSys(info)
		if err != nil || entry.IsDir() || info.Mode()&os.ModeSymlink != 0 || !info.Mode().IsRegular() || !ok || stat.Uid != 0 || stat.Gid != 0 || stat.Nlink != 1 ||
			info.Size() < 0 || info.Size() > 256<<10 || info.Mode().Perm()&0o022 != 0 {
			return errors.New("deployment checkpoint temporary is unsafe")
		}
...
func checkpointStoreUsage(root string) (int, int64, error) {
	entries, err := os.ReadDir(root)
	if err != nil {
		return 0, 0, err
	}
	...
	for _, entry := range entries {
		name := entry.Name()
		if strings.HasPrefix(name, ".solovey-deployment-") {
			return 0, 0, errors.New("deployment checkpoint temporary cleanup is incomplete")
		}
		revision := strings.TrimSuffix(name, ".json")
		if revision == name || !digest(revision) || entry.IsDir() {
			return 0, 0, errors.New("deployment checkpoint store contains an unknown entry")
		}
		info, err := entry.Info()
		stat, ok := itemSys(info)
		if err != nil || info.Mode()&os.ModeSymlink != 0 || !info.Mode().IsRegular() || !ok || stat.Uid != 0 || stat.Gid != 0 || stat.Nlink != 1 ||
			info.Size() < 0 || info.Size() > 256<<10 || info.Mode().Perm()&0o022 != 0 {
			return 0, 0, errors.New("deployment checkpoint store entry is unsafe")
		}
		count++
		bytes += info.Size()
	}
...
func readCheckpoint(revision string) (checkpointV1, error) {
	return readCheckpointAt(checkpointRoot, revision)
}

func readCheckpointAt(root, revision string) (checkpointV1, error) {
	if !digest(revision) {
		return checkpointV1{}, errors.New("deployment checkpoint reference is invalid")
	}
	data, _, err := secureRootFile(filepath.Join(root, revision+".json"), 256<<10)
	if err != nil {
		return checkpointV1{}, err
	}
...
func selectProfile(profile domain.ProfileID) error {
	...
	if err := replaceSymlinkDurably(productionDirectoryEntryOps, activeUnit, path, ".solovey-new", false, 0, 0); err != nil {
		return err
	}
...
func restoreCheckpoint(checkpoint checkpointV1) error {
	if checkpoint.OriginalUnitTarget != "" {
		if err := replaceSymlinkDurably(productionDirectoryEntryOps, activeUnit, checkpoint.OriginalUnitTarget, ".solovey-rollback", true,
			int(checkpoint.OriginalUnitUID), int(checkpoint.OriginalUnitGID)); err != nil {
			return err
		}
	...
	return removeRegularEntryDurably(productionDirectoryEntryOps, profileMarker)
}

type deploymentDirectoryEntryOps struct {
...
var productionDirectoryEntryOps = deploymentDirectoryEntryOps{
	lstat: os.Lstat, remove: os.Remove, symlink: os.Symlink, lchown: os.Lchown, rename: os.Rename, syncDir: syncDir,
}

func replaceSymlinkDurably(ops deploymentDirectoryEntryOps, active, target, temporarySuffix string, setOwner bool, uid, gid int) error {
	if ops.lstat == nil || ops.remove == nil || ops.symlink == nil || ops.lchown == nil || ops.rename == nil || ops.syncDir == nil ||
		!filepath.IsAbs(active) || !filepath.IsAbs(target) || temporarySuffix == "" {
		return errors.New("deployment symlink publication contract is unavailable")
	}
	temporary := active + temporarySuffix
	if err := ops.remove(temporary); err != nil && !errors.Is(err, os.ErrNotExist) {
		return err
	}
	if err := ops.symlink(target, temporary); err != nil {
		return err
	}
	ok := false
	defer func() {
	...
	if setOwner {
		if err := ops.lchown(temporary, uid, gid); err != nil {
			return err
		}
	}
	if err := ops.rename(temporary, active); err != nil {
		return err
	}
	ok = true
	return ops.syncDir(filepath.Dir(active))
}

func removeRegularEntryDurably(ops deploymentDirectoryEntryOps, path string) error {
	if ops.lstat == nil || ops.remove == nil || ops.syncDir == nil || !filepath.IsAbs(path) {
		return errors.New("deployment entry removal contract is unavailable")
	}
...
func migrateLegacyData(checkpoint checkpointV1) error {
	...
	if checkpoint.HardenedRootPresent {
		...
		if err := createOwnedDirectoryDurably(productionDirectoryCreationOps, hardenedDBRoot, 0o700, uid, gid); err != nil {
			return err
		}
	}
	...
	if err := copyStoppedSQLiteGeneration(legacyDBRoot, hardenedDBRoot, uid, gid); err != nil {
		return err
	}
...
type deploymentDirectoryCreationOps struct {
...
var productionDirectoryCreationOps = deploymentDirectoryCreationOps{
	mkdir: os.Mkdir, chown: os.Chown, chmod: os.Chmod, remove: os.Remove, syncDir: syncDir,
}

func createOwnedDirectoryDurably(ops deploymentDirectoryCreationOps, path string, mode os.FileMode, uid, gid int) error {
	if ops.mkdir == nil || ops.chown == nil || ops.chmod == nil || ops.remove == nil || ops.syncDir == nil || !filepath.IsAbs(path) {
		return errors.New("deployment directory creation contract is unavailable")
	}
	...
	cleanup := func(cause error) error {
		...
		syncErr := ops.syncDir(filepath.Dir(path))
...
func copyStoppedSQLiteGeneration(sourceRoot, targetRoot string, uid, gid int) error {
	return copyStoppedSQLiteGenerationWithCopy(sourceRoot, targetRoot, func(source, target string) error {
		return copyRegular(source, target, 0o600, uid, gid)
	})
}

func copyStoppedSQLiteGenerationWithCopy(sourceRoot, targetRoot string, copyFile func(string, string) error) error {
	if copyFile == nil {
		return errors.New("legacy SQLite copy contract is unavailable")
	}
	before, err := snapshotStoppedSQLiteGeneration(sourceRoot)
	if err != nil {
		return err
	}
	for _, file := range before {
		if err := copyFile(filepath.Join(sourceRoot, file.Name), filepath.Join(targetRoot, file.Name)); err != nil {
			return err
		}
	}
	after, err := snapshotStoppedSQLiteGeneration(sourceRoot)
	if err != nil || !sameStoppedSQLiteGeneration(before, after) {
		return errors.New("legacy SQLite generation changed while copying")
	}
	return nil
}

func snapshotStoppedSQLiteGeneration(root string) ([]stoppedSQLiteFile, error) {
	entries, err := os.ReadDir(root)
	if err != nil || len(entries) > 64 {
		return nil, errors.New("legacy database directory is unavailable")
	}
	...
	if main, ok := available["solovey-ui.db"]; !ok || main.IsDir() {
		return nil, errors.New("legacy primary database is unavailable")
	}
	result := make([]stoppedSQLiteFile, 0, 3)
	for _, name := range []string{"solovey-ui.db", "solovey-ui.db-wal", "solovey-ui.db-shm"} {
		...
		info, err := entry.Info()
		stat, statOK := itemSys(info)
		if err != nil || entry.IsDir() || info.Mode()&os.ModeSymlink != 0 || !info.Mode().IsRegular() || !statOK || stat.Uid != 0 ||
			stat.Nlink != 1 || stat.Size < 0 || stat.Size > 4<<30 {
			return nil, errors.New("legacy database entry is unsafe")
		}
		result = append(result, stoppedSQLiteFile{Name: name, Device: uint64(stat.Dev), Inode: stat.Ino, Size: stat.Size,
			MTimeSec: int64(stat.Mtim.Sec), MTimeNSec: int64(stat.Mtim.Nsec), CTimeSec: int64(stat.Ctim.Sec), CTimeNSec: int64(stat.Ctim.Nsec),
			UID: stat.Uid, GID: stat.Gid, Mode: stat.Mode, LinkCount: uint32(stat.Nlink)})
	}
	return result, nil
}

func sameStoppedSQLiteGeneration(left, right []stoppedSQLiteFile) bool {
...
func copyRegular(source, target string, mode os.FileMode, uid, gid int) error {
	return copyRegularWithOps(productionDeploymentCopyOps, source, target, mode, uid, gid)
}

type deploymentCopyOps struct {
	createTemp func(string, string) (deploymentAtomicFile, error)
...
var productionDeploymentCopyOps = deploymentCopyOps{
	createTemp: func(directory, pattern string) (deploymentAtomicFile, error) {
		return os.CreateTemp(directory, pattern)
	},
	copy: io.Copy, remove: os.Remove, rename: os.Rename, syncDir: syncDir,
}

func copyRegularWithOps(ops deploymentCopyOps, source, target string, mode os.FileMode, uid, gid int) error {
	if ops.createTemp == nil || ops.copy == nil || ops.remove == nil || ops.rename == nil || ops.syncDir == nil {
		return errors.New("legacy SQLite copy contract is unavailable")
	}
	...
	temporary, err := ops.createTemp(directory, ".solovey-db-migration-")
	if err != nil {
		return err
	}
	name := temporary.Name()
	...
	written, err := ops.copy(temporary, io.LimitReader(input, before.Size+1))
	...
	if err := ops.rename(name, target); err != nil {
		return err
	}
	cleanupOK = true
...
type deploymentAtomicFile interface {
	Name() string
...
type deploymentAtomicOps struct {
	createTemp func(string, string) (deploymentAtomicFile, error)
	remove     func(string) error
	rename     func(string, string) error
	syncDir    func(string) error
...
var productionDeploymentAtomicOps = deploymentAtomicOps{
	createTemp: func(directory, pattern string) (deploymentAtomicFile, error) {
		return os.CreateTemp(directory, pattern)
	},
	remove:  os.Remove,
...
func atomicWrite(path string, data []byte, mode os.FileMode, uid, gid int) error {
	return atomicWriteWithOps(productionDeploymentAtomicOps, path, data, mode, uid, gid)
}

func atomicWriteWithOps(ops deploymentAtomicOps, path string, data []byte, mode os.FileMode, uid, gid int) error {
	if ops.createTemp == nil || ops.remove == nil || ops.rename == nil || ops.syncDir == nil {
		return errors.New("deployment atomic filesystem contract is unavailable")
	}
	...
	temporary, err := ops.createTemp(directory, ".solovey-deployment-")
	if err != nil {
		return err
	}
	name := temporary.Name()
...
func ensureRootDirectory(path string, mode os.FileMode) error {
	return ensureRootDirectoryWithSync(path, mode, syncDir)
}

func ensureRootDirectoryWithSync(path string, mode os.FileMode, synchronize func(string) error) error {
	if synchronize == nil {
		return errors.New("deployment root durability barrier is unavailable")
	}
	...
	// Sync the containing directory on every admission. This covers both the
	// first mkdir and a prior attempt whose parent sync failed before success
	// could be reported.
	return synchronize(filepath.Dir(path))
}
...
func fixedBinary(paths ...string) (*executableobject.Object, error) {
	for _, path := range paths {
	...
	return nil, errFixedRootOwnedExecutableUnavailable
}
