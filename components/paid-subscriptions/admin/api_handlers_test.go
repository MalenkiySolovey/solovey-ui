...
func withTelegramActions(t *testing.T, broadcast BroadcastFunc, refund RefundFunc) {
	t.Helper()
	telegramActions.Lock()
	oldBroadcast := telegramActions.broadcast
	oldRefund := telegramActions.refund
	...
	t.Cleanup(func() {
		telegramActions.Lock()
		telegramActions.broadcast = oldBroadcast
		telegramActions.refund = oldRefund
		telegramActions.Unlock()
	})
}
...
func TestRefundHandlerValidationAndSuccess(t *testing.T) {
	db := paidtest.OpenDatabase(t)
	if err := paidcore.EnsureSchema(db); err != nil {
		t.Fatalf("EnsureSchema: %v", err)
	}
	withTelegramActions(t, nil, func(_ context.Context, orderID uint, _ bool) (string, error) {
	...
	order := paidcore.PaymentOrder{ClientId: 1, TariffId: 1, Provider: "yookassa", Amount: 10000, Currency: "RUB", Status: paidcore.StatusPaid, IdempotencyKey: "h-refund"}
	db.Create(&order)
