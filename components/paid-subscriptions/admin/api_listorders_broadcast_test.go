...
func TestListOrdersHandler(t *testing.T) {
	...
	order := paidcore.PaymentOrder{ClientId: client.Id, TariffId: 1, Provider: "stars", Amount: 100, Currency: "XTR", Status: paidcore.StatusPaid, IdempotencyKey: "h-list"}
...
func TestBroadcastHandlerValidation(t *testing.T) {
	...
	withTelegramActions(t, func(context.Context, string) (int, int, error) {
		return 0, 0, errors.New("bot token not configured")
	}, nil)
	h := newTestHandlers()
