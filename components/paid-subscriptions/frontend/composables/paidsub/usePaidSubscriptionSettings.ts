...
import {
  ...
  pickPaidSubSettings,
  type PaidSubSettingsMap as SMap,
} from '../../views/paidSubSettingsPayload'
import { paidSubTransportModes } from './tableConfig'
...
export const usePaidSubscriptionSettings = (loading: Ref<boolean>) => {
  ...
  const saveSettings = async () => {
    ...
    if (msg.success) {
      push.success({ title: i18n.global.t('success'), message: i18n.global.t('paidSub.title'), duration: 4000 })
      if (msg.obj?.settings) {
