...
// Package telegramapi owns Telegram component operations exposed through the HTTP API.
package telegramapi
...
func (a *Handler) runTelegramBackupManual(c *gin.Context) {
	...
	backupService := telegramservice.TelegramBackupService{
		Settings: a.Settings,
		Telegram: a.Telegram,
		Audit: func(record telegramservice.AuditRecord) error {
			return a.AuditService.Record(service.AuditEvent{
				Actor:    record.Actor,
				Event:    record.Event,
	...
	ctx := telegramservice.ContextWithTelegramBackupActor(c.Request.Context(), a.Actor(c))
