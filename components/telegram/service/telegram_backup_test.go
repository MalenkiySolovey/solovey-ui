...
func TestTelegramBackupRunOnceDisabledAndMissingPassphrase(t *testing.T) {
	...
	result := (&telegramservice.TelegramBackupService{Settings: testTelegramSettings{}, Audit: testTelegramBackupAudit}).RunOnce(context.Background(), telegramservice.TelegramBackupTriggerManual)
...
func TestTelegramBackupRunOnceSuccessSendsEnvelopeAndAuditsSizes(t *testing.T) {
	...
	backupService := &telegramservice.TelegramBackupService{Settings: testTelegramSettings{}, Telegram: &telegramservice.Service{Settings: testTelegramSettings{}, Client: &http.Client{Transport: rt, Timeout: time.Second}}, Audit: testTelegramBackupAudit}

	ctx := telegramservice.ContextWithTelegramBackupActor(context.Background(), "admin")
	result := backupService.RunOnce(ctx, telegramservice.TelegramBackupTriggerManual)
	if !result.Success || result.Filename == "" || result.Trigger != telegramservice.TelegramBackupTriggerManual {
		t.Fatalf("unexpected result: %#v", result)
	}
	document := telegramDocumentFromCapture(t, rt)
	if !backupenvelope.IsEnvelope(document) {
...
func TestTelegramBackupRunOnceConcurrentGuard(t *testing.T) {
	...
	done := make(chan telegramservice.TelegramBackupResult, 1)
	go func() {
		done <- backupService.RunOnce(context.Background(), telegramservice.TelegramBackupTriggerManual)
	}()
	select {
	case <-started:
...
func configureTelegramBackupSettings(t *testing.T, settingService *coreservice.SettingService, cfg telegramBackupSettings) {
	t.Helper()
...
func testTelegramBackupAudit(record telegramservice.AuditRecord) error {
