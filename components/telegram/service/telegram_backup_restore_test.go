...
func TestRestoreEndpointAcceptsPlaintextAndTelegramBackupEnvelope(t *testing.T) {
	...
	t.Run("envelope", func(t *testing.T) {
		initRestoreEndpointTestDB(t)
		registerTelegramBackupImportCodecForTest(t)
		...
		envelope, err := backupenvelope.Build(backup, []byte(passphrase))
		if err != nil {
			t.Fatal(err)
		}
		if err := setRestoreEndpointMarker("live-before-import"); err != nil {
...
func registerTelegramBackupImportCodecForTest(t *testing.T) {
	t.Helper()
	dbtransferhttp.ResetBackupCodecsForTest()
	unregister := dbtransferhttp.RegisterBackupImportCodec("test-telegram", dbtransferhttp.BackupImportCodec{
...
func initRestoreEndpointTestDB(t *testing.T) {
	...
	if _, err := (&coreservice.SettingService{}).GetAllSetting(); err != nil {
		t.Fatal(err)
	}
...
func performRestoreEndpointRequest(t *testing.T, content []byte, passphrase string) *httptest.ResponseRecorder {
	...
	handler := &dbtransferhttp.Handler{
		...
		Audit: func(_ *gin.Context, actor, event, resource, severity string, details map[string]any) {
			_ = (&coreservice.AuditService{}).Record(coreservice.AuditEvent{
