...
type CPUAlertJob struct {
	service.ServerService
	settingsReader telegramsettings.Reader
...
func NewCPUAlertJob() *CPUAlertJob {
	job := &CPUAlertJob{settingsReader: telegramsettings.Reader{}}
	job.cpuPercent = job.ServerService.GetCpuPercent
	job.settings = func() (bool, float64) {
		enabled, err := job.settingsReader.GetTelegramNotifyCpu()
		if err != nil || !enabled {
			return false, 0
		}
		threshold, err := job.settingsReader.GetTelegramCpuThreshold()
		if err != nil || threshold <= 0 {
			threshold = 90
		}
		return true, float64(threshold)
	}
	job.notify = service.NotifyPanelEvent
	return job
}

func (j *CPUAlertJob) Run() {
