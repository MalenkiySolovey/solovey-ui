...
func TestExtendSlowRequestDeadlinesUsesConn(t *testing.T) {
	...
	conn := &recordingConn{}
	ctx := httpconn.SaveContext(context.Background(), conn)
...
// TestHTTPConnContextRoundTrip ensures the ConnContext hook value is retrievable
// exactly as stored.
func TestHTTPConnContextRoundTrip(t *testing.T) {
	conn := &recordingConn{}
	got, ok := httpconn.FromContext(httpconn.SaveContext(context.Background(), conn))
	if !ok || got != conn {
		t.Fatalf("httpconn.FromContext = (%v, %v), want (%v, true)", got, ok, conn)
	}
	if _, ok := httpconn.FromContext(context.Background()); ok {
		t.Fatal("httpconn.FromContext returned ok for a context without a conn")
	}
}
