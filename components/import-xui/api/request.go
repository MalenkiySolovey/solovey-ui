...
func RegisterResetHook() func() {
	dbhooks.RegisterResetHook("api.xui_rates", ResetRateLimits)
	return func() {
		dbhooks.RegisterResetHook("api.xui_rates", nil)
	}
}
...
//
// It sets the deadline on the raw net.Conn from the shared ConnContext hook. The
...
func extendSlowRequestDeadlines(c *gin.Context) {
	...
	if conn, ok := httpconn.FromContext(c.Request.Context()); ok {
...
func (a *Handler) enforceRateLimit(c *gin.Context) bool {
	...
	if !xuiRequestRateLimiter.Allow(key).Allowed {
		a.Audit(c, a.Actor(c), "panel_import_failed", "database", service.AuditSeverityWarn, map[string]any{"reason": "rate_limited"})
