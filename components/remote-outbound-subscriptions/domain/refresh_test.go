...
func TestRefreshSubscriptionOutboundsCreatesConnectionsAndDefaultGroup(t *testing.T) {
	...
	var storedSubscription RemoteOutboundSubscription
...
func TestRefreshSubscriptionOutboundsUpdatesAndPrunesStaleConnections(t *testing.T) {
	db := newRemoteSyncDB(t)
	sub := RemoteOutboundSubscription{Name: "Remote", TagPrefix: "ros-"}
	if err := db.Create(&sub).Error; err != nil {
		t.Fatal(err)
	...
	var count int64
	if err := db.Model(&RemoteOutboundConnection{}).Where("source_key = ?", "label:nodeb").Count(&count).Error; err != nil {
		t.Fatal(err)
	}
	if count != 0 {
		t.Fatalf("node B should be pruned from subscription, count=%d", count)
	}
}
...
func TestRefreshSubscriptionOutboundsMigratesLegacyTypedLabelSourceKey(t *testing.T) {
	...
	connection := RemoteOutboundConnection{
	...
	if connections[0].Id != connection.Id || connections[0].SourceKey != "label:nodea" {
...
func TestRefreshSubscriptionOutboundsHidesOldDuplicateAfterSourceKeyMigration(t *testing.T) {
	...
	current := RemoteOutboundConnection{
		SubscriptionId: sub.Id,
		Name:           "Node A",
	...
	legacy := RemoteOutboundConnection{
		SubscriptionId: sub.Id,
		Name:           "Node A",
		SourceKey:      "label:vless:nodea",
		Type:           "vless",
	...
	if result.Created != 0 || result.MarkedMissing != 1 {
		t.Fatalf("refresh result = %#v, want old duplicate pruned", result)
	}
	var stored RemoteOutboundSubscription
	if err := db.Preload("Connections").First(&stored, sub.Id).Error; err != nil {
		t.Fatal(err)
	}
	if len(stored.Connections) != 1 {
		t.Fatalf("stored connections = %#v, want only current", stored.Connections)
	}
	subscriptions := []RemoteOutboundSubscription{stored}
...
func TestRefreshSubscriptionOutboundsTreatsDifferentNormalizedNameAsNewConnection(t *testing.T) {
	...
	if len(connections) != 1 {
		t.Fatalf("connections = %d, want only renamed row: %#v", len(connections), connections)
	}
	if connections[0].SourceKey != "label:noderenamed" {
		t.Fatalf("new connection state = %#v", connections[0])
	}
}
...
func TestRefreshSubscriptionOutboundsTreatsRenamedGroupAsNewConnection(t *testing.T) {
	...
	if len(connections) != 2 {
		t.Fatalf("connections = %d, want node + new group: %#v", len(connections), connections)
	}
	newGroup := connections[1]
	if newGroup.Name != "Auto Renamed" || newGroup.SourceKey != "label:autorenamed" {
...
func TestRefreshFetchedXrayBalancerSyncsRootURLTestWithDependencies(t *testing.T) {
  ...
  "routing": {
	...
	var groupConnection RemoteOutboundConnection
	...
	var defaultLinks []RemoteOutboundGroupConnection
...
func TestRefreshStaleSyncedConnectionMarksLinkedOutboundOnly(t *testing.T) {
	db := newRemoteSyncDB(t)
	sub := RemoteOutboundSubscription{Name: "Remote", TagPrefix: "ros-"}
	if err := db.Create(&sub).Error; err != nil {
		t.Fatal(err)
	...
	var nodeB RemoteOutboundConnection
	if err := db.Unscoped().Where("source_key = ?", "label:nodeb").First(&nodeB).Error; err == nil {
		t.Fatalf("stale connection should be pruned from subscription: %#v", nodeB)
	}
	var outbound model.Outbound
	if err := db.Where("tag = ?", "ros-Node-B").First(&outbound).Error; err != nil {
		t.Fatal(err)
	}
	...
	var restored RemoteOutboundConnection
	if err := db.Where("source_key = ?", "label:nodeb").First(&restored).Error; err != nil {
		t.Fatal(err)
	}
	if restored.OutboundId == nil || *restored.OutboundId != outbound.Id {
		t.Fatalf("restored connection did not reclaim missing outbound: %#v outbound=%#v", restored, outbound)
	}
...
func TestDueSubscriptions(t *testing.T) {
	db := newRemoteSyncDB(t)
	fixtures := []RemoteOutboundSubscription{
