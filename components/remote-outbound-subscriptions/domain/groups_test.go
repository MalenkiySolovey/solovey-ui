...
func TestHydrateConnectionGroupIDs(t *testing.T) {
	db := newRemoteDB(t)
	sub := RemoteOutboundSubscription{Name: "sub"}
	if err := db.Create(&sub).Error; err != nil {
		t.Fatal(err)
	}
	groupA := RemoteOutboundGroup{SubscriptionId: sub.Id, Name: "A"}
	groupB := RemoteOutboundGroup{SubscriptionId: sub.Id, Name: "B"}
	if err := db.Create(&[]*RemoteOutboundGroup{&groupA, &groupB}).Error; err != nil {
		t.Fatal(err)
	}
	connection := RemoteOutboundConnection{SubscriptionId: sub.Id, Name: "node", SourceKey: "node", OutboundTag: "ros-node"}
	...
	subscriptions := []RemoteOutboundSubscription{{
		Id:          sub.Id,
		Connections: []RemoteOutboundConnection{connection},
	}}
...
func TestReconcileGroupStatesDisablesIncompleteGroup(t *testing.T) {
	...
	group := RemoteOutboundGroup{SubscriptionId: sub.Id, Name: "sync", OutboundEnabled: true}
	...
	var stored RemoteOutboundGroup
...
func TestFilterUsableConnectionsAndAllSynced(t *testing.T) {
	connections := []RemoteOutboundConnection{
		...
		{Enabled: false, Synced: true},
		{Enabled: true, Synced: true},
	}
	active := FilterUsableConnections(connections)
	if len(active) != 2 {
		t.Fatalf("active = %#v, want two enabled connections", active)
	}
