...
func MarkRefreshError(tx *gorm.DB, subscriptionID uint, message string, now int64) error {
	return tx.Model(&RemoteOutboundSubscription{}).
...
func RefreshFetchedSubscription(tx *gorm.DB, subscription *RemoteOutboundSubscription, fetched *FetchedSubscription, now int64) (RefreshResult, bool, error) {
	if fetched == nil {
...
func refreshSubscriptionOutbounds(tx *gorm.DB, subscription *RemoteOutboundSubscription, outbounds []map[string]interface{}, canonicalSnapshot subcanonical.Snapshot, collectionSnapshot json.RawMessage, now int64) (RefreshResult, bool, error) {
	...
	if err := tx.Model(&RemoteOutboundSubscription{}).Where("id = ?", subscription.Id).Updates(updates).Error; err != nil {
		return result, false, err
	}
...
func refreshSeenConnections(tx *gorm.DB, subscription *RemoteOutboundSubscription, defaultGroupID uint, outbounds []map[string]interface{}, canonicalSnapshot subcanonical.Snapshot, now int64, result *RefreshResult) (map[string]struct{}, bool, error) {
	...
	for index, outbound := range outbounds {
		...
		if (connection.Synced || shouldSync) && connection.Enabled {
...
func upsertRefreshConnection(tx *gorm.DB, subscription *RemoteOutboundSubscription, defaultGroupID uint, outbound map[string]interface{}, canonicalConnection *subcanonical.Connection, index int, seen map[string]struct{}, now int64, result *RefreshResult) (RemoteOutboundConnection, bool, error) {
	normalized, err := NormalizeOutboundWithCanonical(outbound, index, canonicalConnection)
	if err != nil {
		return RemoteOutboundConnection{}, false, err
	}
	...
	case errors.Is(err, gorm.ErrRecordNotFound):
		...
		if legacy != nil {
			...
			if sortOrder <= 0 {
				nextSortOrder, err := entityorder.Next(tx, &RemoteOutboundConnection{})
				if err != nil {
					return RemoteOutboundConnection{}, false, err
				}
			...
			if outboundID, err := reusableRemoteMissingOutboundID(tx, tag); err != nil {
				return RemoteOutboundConnection{}, false, err
			} else if outboundID != 0 {
				connection.OutboundId = &outboundID
			}
			result.Created++
		}
	default:
...
func findConnectionByLegacySourceKey(tx *gorm.DB, subscriptionID uint, normalized NormalizedOutbound) (*RemoteOutboundConnection, error) {
...
func pruneStaleConnections(tx *gorm.DB, subscriptionID uint, seen map[string]struct{}, now int64, result *RefreshResult) (bool, error) {
	var existing []RemoteOutboundConnection
	...
	for _, connection := range existing {
		...
		if connection.Synced {
			if err := MarkConnectionOutboundMissing(tx, &connection, "not present in latest successful refresh", now); err != nil {
				return false, err
			}
			syncedChanged = true
		}
		if err := tx.Where("connection_id = ?", connection.Id).Delete(&RemoteOutboundGroupConnection{}).Error; err != nil {
			return false, err
		}
...
func DueSubscriptions(tx *gorm.DB, now int64) ([]RemoteOutboundSubscription, error) {
	var subscriptions []RemoteOutboundSubscription
	err := tx.
