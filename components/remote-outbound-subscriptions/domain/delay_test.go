...
func TestCheckConnectionRecordsSkipsDisabledOnly(t *testing.T) {
	results := CheckConnectionRecords(context.Background(), []RemoteOutboundConnection{
	...
	if results[1].Skipped {
		t.Fatalf("enabled result = %#v", results[1])
	}
}

func TestCheckSubscriptionOrdersBySortOrder(t *testing.T) {
	db := newRemoteSyncDB(t)
	sub := RemoteOutboundSubscription{Name: "Remote", Url: "https://example.com/sub"}
	if err := db.Create(&sub).Error; err != nil {
		t.Fatal(err)
	}
	for _, connection := range []RemoteOutboundConnection{
...
func TestCheckOutboundsIncludesConvertedGroupMembers(t *testing.T) {
	...
	otherNode := RemoteOutboundConnection{
		SubscriptionId: sub.Id,
		SortOrder:      30,
		Name:           "proxy-b",
	...
	otherGroup := RemoteOutboundConnection{
		SubscriptionId: sub.Id,
		SortOrder:      40,
		Name:           "other-auto",
