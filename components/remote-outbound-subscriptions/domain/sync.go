...
func SyncConnectionToOutbound(tx *gorm.DB, connection *RemoteOutboundConnection, requireUsable bool) error {
	return syncConnectionToOutbound(tx, connection, requireUsable, map[uint]struct{}{})
}

func UnsyncConnectionsForDeletedOutbound(tx *gorm.DB, tag string) error {
	if !tx.Migrator().HasTable(&RemoteOutboundConnection{}) {
		return nil
	}
	var linkedConnections []RemoteOutboundConnection
	if err := tx.Where("outbound_tag = ?", tag).Find(&linkedConnections).Error; err != nil {
		return err
	}
	if tx.Migrator().HasTable(&RemoteOutboundGroup{}) && tx.Migrator().HasTable(&RemoteOutboundGroupConnection{}) {
		for _, connection := range linkedConnections {
			if err := tx.Model(&RemoteOutboundGroup{}).
	...
	return tx.Model(&RemoteOutboundConnection{}).
...
func syncConnectionToOutbound(tx *gorm.DB, connection *RemoteOutboundConnection, requireUsable bool, visited map[uint]struct{}) error {
	if connection == nil {
...
func syncSingleConnectionToOutbound(tx *gorm.DB, connection *RemoteOutboundConnection, requireUsable bool) error {
	if requireUsable {
		switch {
	...
	if outbound.Id != 0 {
		if connection.OutboundId == nil || outbound.Id != *connection.OutboundId {
			var linked int64
			if err := tx.Model(&RemoteOutboundConnection{}).
...
func MarkConnectionOutboundMissing(tx *gorm.DB, connection *RemoteOutboundConnection, reason string, now int64) error {
	if connection == nil {
		return nil
	}
	var outbound model.Outbound
...
func remoteMissingSource(tx *gorm.DB, connection RemoteOutboundConnection) string {
	var subscription RemoteOutboundSubscription
...
func connectionOutboundConfigForSync(tx *gorm.DB, connection RemoteOutboundConnection) (json.RawMessage, error) {
...
func remoteConnectionTagMap(connections []RemoteOutboundConnection) map[string]string {
...
func UnsyncConnectionFromOutbound(tx *gorm.DB, connection *RemoteOutboundConnection) error {
	now := time.Now().Unix()
...
func deleteLinkedOutbound(tx *gorm.DB, connection RemoteOutboundConnection) (bool, error) {
	if connection.OutboundId == nil || *connection.OutboundId == 0 {
		return false, nil
	}
...
func outboundMatchesRemoteConnection(tx *gorm.DB, connection RemoteOutboundConnection, outbound model.Outbound) (bool, error) {
	config, err := connectionOutboundConfigForSync(tx, connection)
	if err != nil {
		return false, err
	}
