...
func DeleteGroup(tx *gorm.DB, id uint, now int64) (RemoteOutboundGroup, error) {
	var group RemoteOutboundGroup
	...
	var affected []RemoteOutboundGroupConnection
	if err := tx.Where("group_id = ?", group.Id).Find(&affected).Error; err != nil {
		return group, err
	}
	if err := tx.Where("group_id = ?", group.Id).Delete(&RemoteOutboundGroupConnection{}).Error; err != nil {
		return group, err
	}
...
func MoveConnectionToGroup(tx *gorm.DB, connectionID uint, groupID uint, now int64) (RemoteOutboundConnection, error) {
...
func removeGroupConnections(tx *gorm.DB, group RemoteOutboundGroup, defaultGroupID uint, removed []uint) (bool, error) {
	if err := tx.
...
func addGroupConnections(tx *gorm.DB, group RemoteOutboundGroup, added []uint, now int64) (bool, error) {
	coreRestart := false
...
func disableGroupOutbounds(tx *gorm.DB, group *RemoteOutboundGroup, connections []RemoteOutboundConnection, now int64, result *GroupActionResult) error {
...
func enableGroupOutbounds(tx *gorm.DB, group *RemoteOutboundGroup, active []RemoteOutboundConnection, now int64, result *GroupActionResult) error {
