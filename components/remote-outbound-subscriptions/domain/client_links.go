...
func OutboundsForClientLinksWithOptions(db *gorm.DB, rawLinks json.RawMessage, options ClientConversionOptions) ([]map[string]interface{}, []string, error) {
	...
	connections := make([]RemoteOutboundConnection, 0)
...
func clientLinkSelectionConnections(db *gorm.DB, selection clientLinkSelection) ([]RemoteOutboundConnection, error) {
...
func clientRemoteGroupConnections(db *gorm.DB, groupID uint) ([]RemoteOutboundConnection, error) {
	...
	err := db.
		...
		Where("remote_outbound_connections.enabled = ?", true).
...
func clientRemoteSubscriptionConnections(db *gorm.DB, subscriptionID uint) ([]RemoteOutboundConnection, error) {
	var connections []RemoteOutboundConnection
	err := db.
		Model(&RemoteOutboundConnection{}).
		Joins("JOIN remote_outbound_subscriptions ON remote_outbound_subscriptions.id = remote_outbound_connections.subscription_id").
	...
	subscriptions := []RemoteOutboundSubscription{{Connections: connections}}
	FilterVisibleConnections(subscriptions)
	return subscriptions[0].Connections, nil
}
func appendClientTargetExtras(db *gorm.DB, outbounds []map[string]interface{}, tags []string, connections []RemoteOutboundConnection, tagMap map[string]string, target string) ([]map[string]interface{}, []string, error) {
