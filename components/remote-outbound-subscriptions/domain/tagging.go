...
func RetagSubscriptionConnections(tx *gorm.DB, subscription *RemoteOutboundSubscription) (bool, error) {
...
func UniqueOutboundTag(tx *gorm.DB, subscription RemoteOutboundSubscription, name string, connectionID uint) (string, error) {
...
func OutboundTagAvailable(tx *gorm.DB, tag string, connectionID uint) (bool, error) {
	...
	if outbounds > 0 {
		if connectionID == 0 {
			return reusableRemoteMissingOutboundExists(tx, tag)
		}
		var linked int64
	...
	query := tx.Model(&RemoteOutboundConnection{}).Where("outbound_tag = ?", tag)
	if connectionID != 0 {
...
func reusableRemoteMissingOutboundExists(tx *gorm.DB, tag string) (bool, error) {
	outboundID, err := reusableRemoteMissingOutboundID(tx, tag)
	return outboundID != 0, err
}

func reusableRemoteMissingOutboundID(tx *gorm.DB, tag string) (uint, error) {
	var outbound model.Outbound
	if err := tx.Where("tag = ? AND remote_missing = ?", tag, true).First(&outbound).Error; err != nil {
	...
	var linked int64
	if err := tx.Model(&RemoteOutboundConnection{}).
		Where("outbound_id = ? OR outbound_tag = ?", outbound.Id, tag).
		Count(&linked).Error; err != nil {
		return 0, err
	}
	if linked > 0 {
