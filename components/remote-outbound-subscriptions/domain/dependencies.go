...
func ReconcileDerivedGroupDependencies(tx *gorm.DB, subscriptionID uint, defaultGroupID uint) error {
	...
	if err := tx.
		...
		Delete(&RemoteOutboundGroupConnection{}).Error; err != nil {
		return err
	}
	return SyncLegacyGroupIDs(tx, subscriptionID)
}

func FilterVisibleConnections(subscriptions []RemoteOutboundSubscription) {
...
func syncGroupDependencyOutbounds(tx *gorm.DB, group RemoteOutboundConnection, requireUsable bool, visited map[uint]struct{}) error {
...
func ExpandConnectionsWithGroupDependencies(tx *gorm.DB, connections []RemoteOutboundConnection) ([]RemoteOutboundConnection, error) {
	result := make([]RemoteOutboundConnection, 0, len(connections))
...
func unsyncGroupDependenciesIfUnused(tx *gorm.DB, group RemoteOutboundConnection) (int, error) {
	members, err := groupDependencyConnections(tx, group)
	if err != nil {
		return 0, err
	}
...
func dependencyUsedBySyncedGroup(tx *gorm.DB, member RemoteOutboundConnection) (bool, error) {
...
func groupDependencyConnections(tx *gorm.DB, group RemoteOutboundConnection) ([]RemoteOutboundConnection, error) {
	...
	result := make([]RemoteOutboundConnection, 0, len(refs))
	seen := map[uint]struct{}{}
...
func groupDependencyIDs(connections []RemoteOutboundConnection) map[uint]struct{} {
	return technicalDependencyIDs(connections)
}

func technicalDependencyIDs(connections []RemoteOutboundConnection) map[uint]struct{} {
...
func connectionIsTechnicalDependency(connection RemoteOutboundConnection) bool {
...
func groupMemberRefs(connection RemoteOutboundConnection) []string {
...
func connectionMatchesMemberRef(connection RemoteOutboundConnection, ref string) bool {
