...
func collectedProfile(subscription remotesub.RemoteOutboundSubscription) []CollectedProfileBlock {
	...
	for _, memberRef := range connection.GroupMembers {
		...
		if !ok {
			block.Connections = append(block.Connections, CollectedProfileBlock{
				Name: memberRef,
				Type: "unresolved group reference",
...
func collectedSummary(subscription remotesub.RemoteOutboundSubscription, profile []CollectedProfileBlock) string {
	if len(profile) == 0 {
