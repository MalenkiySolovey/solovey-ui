...
func (a *ApiService) handleSettingsSaveError(c *gin.Context, actor string, obj string, err error) bool {
	if obj != "settings" {
		return false
	}
	...
	a.recordAudit(c, actor, event, "settings", service.AuditSeverityWarn, map[string]any{
		"reason": err.Error(),
	})
	if invalidSettingKey {
...
func (a *ApiService) recordSettingsSaveSucceeded(c *gin.Context, actor string, obj string, action string) {
	if obj != "settings" {
		return
	}
	a.recordAudit(c, actor, "settings_save_succeeded", "settings", service.AuditSeverityInfo, map[string]any{
		"action": action,
	})
}
