...
func init() {
	service.RegisterAPITokenScopeProvider(func() []string { return []string{"telegram"} })
}
...
func TestComponentRouteTelegramBackupRequiresComponentOrAdminScope(t *testing.T) {
	...
	recorder := performComponentTelegramBackupRequest(router, "/apiv2/telegram/backup", readToken)
	...
	recorder = performComponentTelegramBackupRequest(router, "/apiv2/telegram/backup/run", telegramToken)
	if recorder.Code == http.StatusForbidden {
...
func TestComponentRouteTelegramBackupDisabledFailureAuditsWithoutKey(t *testing.T) {
	...
	recorder := performComponentTelegramBackupRequest(router, "/apiv2/telegram/backup", adminToken)
	if recorder.Code != http.StatusServiceUnavailable {
		t.Fatalf("unexpected status: %d", recorder.Code)
	}
	assertComponentTelegramBackupFailure(t, recorder, "disabled")
...
func TestComponentRouteTelegramBackupManualRoutesShareRateLimitBucket(t *testing.T) {
	resetRateLimitState()
	...
	router, cookies, csrf := newComponentTelegramBackupFullRouter(t, settingService)
	requests := []struct {
		method string
		path   string
		token  string
	...
	for i, req := range requests {
		recorder := performComponentTelegramBackupFullRequest(router, req.method, req.path, req.token, req.csrf, cookies...)
		if i < componentManualActionRateLimitMax {
			if recorder.Code != http.StatusServiceUnavailable {
		...
		assertComponentTelegramBackupFailure(t, recorder, "rate_limited")
		if strings.Contains(recorder.Body.String(), "backupKey") {
...
func TestComponentRouteTelegramBackupBrowserRouteRequiresCSRF(t *testing.T) {
	settingService := initSessionTestDB(t)
	router, cookies, _ := newComponentTelegramBackupFullRouter(t, settingService)
	recorder := performComponentTelegramBackupFullRequest(router, http.MethodPost, "/api/telegram/backup/run", "", "", cookies...)
	if recorder.Code != http.StatusForbidden {
...
func newComponentTelegramBackupFullRouter(t *testing.T, settingService *service.SettingService) (*gin.Engine, []*http.Cookie, string) {
	...
	csrfRecorder := performComponentTelegramBackupFullRequest(router, http.MethodGet, "/api/csrf", "", "", cookies...)
...
func performComponentTelegramBackupFullRequest(router *gin.Engine, method string, path string, token string, csrf string, cookies ...*http.Cookie) *httptest.ResponseRecorder {
...
func performComponentTelegramBackupRequest(router *gin.Engine, path string, token string) *httptest.ResponseRecorder {
	recorder := httptest.NewRecorder()
...
func assertComponentTelegramBackupFailure(t *testing.T, recorder *httptest.ResponseRecorder, wantClass string) {
	t.Helper()
	var msg Msg
