...
func TestPrepareDatabaseImportFileDecryptsBackupPassphraseAlias(t *testing.T) {
	...
	envelope, err := service.BuildTelegramBackupEnvelope(plaintext, passphrase)
	if err != nil {
		t.Fatal(err)
	}
	...
	prepared, ok := (&ApiService{}).prepareDatabaseImportFile(c, memoryMultipartFile{Reader: bytes.NewReader(envelope)})
	if !ok {
